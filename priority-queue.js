...
class PriorityQueuePage {
    ...
    getQuestionData(questionId) {
        const questionsData = {
            'largest': {
                description: 'Design a class to find the kth largest integer in a stream of integer nums, including duplicates. E.g. the 2nd largest from [1, 2, 3, 3] is 3. The stream is not necessarily sorted. Implement the following methods: 1) constructor(int k, int[] nums) - Initializes the object given an integer k and the stream of integers nums. 2) int add(int val) - Adds the integer val to the stream and returns the kth largest integer in the stream.',
                complexity: 'Easy',
                examples: [ 'Input: ["KthLargest", [3, [1, 2, 3, 3]], "add", [3], "add", [5], "add", [6], "add", [7], "add", [8]]                Output: [null, 3, 3, 3, 5, 6]' ]
            },
            'largest-solution-optimal': {
                description: 'Optimal solution with priority queue. Time complexity: O(m*log(k))',
                complexity: 'Solution',
                examples: [],
